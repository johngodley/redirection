...
/*			$this->monitor_category = $options['monitor_category'];
	...
	function post_changed ($id)
	{
		if (isset($_POST['redirection_slug'])) {
			...
			if ($newslug != $oldslug && strlen ($oldslug) > 0 && ( strpos( $oldslug, '?p=' ) === false ) && ($post->post_status == 'publish' || $post->post_status == 'static') && $_POST['redirection_status'] != 'draft' && $_POST['redirection_status'] != 'pending' && $newslug != '/' && $oldslug != '/' )
			{
