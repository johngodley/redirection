...
abstract class Red_Log {
	...
	private static function get_export_csv_data_for_rows( array $rows ) {
		...
		fputcsv( $stdout, static::get_csv_header(), escape: '\\' );

		foreach ( $rows as $row ) {
			fputcsv( $stdout, array_map( [ $sanitizer, 'escape' ], static::get_csv_row( $row ) ), escape: '\\' );
		}
	...
	private static function get_custom_export_csv_data( array $rows, array $display_selected, array $params = [] ) {
		...
		fputcsv( $stdout, array_map( [ static::class, 'get_export_field_label' ], $fields ), escape: '\\' );

		foreach ( $rows as $row ) {
			$mapped_row = self::filter_export_row( static::map_export_row( $row ), $fields );
			fputcsv( $stdout, array_map( [ $sanitizer, 'escape' ], array_values( $mapped_row ) ), escape: '\\' );
		}
