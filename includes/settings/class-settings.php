...
use Redirection\Request\Ip;
use Redirection\Request\Request;
use Redirection\Request\Headers;
use Redirection\Url\SourceFlags;

/**
 * Redirection settings manager - pure static utility class.
 *
 * Usage: Settings::get() returns a fully typed array.
 *
...
class Settings {
	 ...
	 * Mirrors legacy Settings::get() behavior.
	 *
	 * @phpstan-return RedirectionOptions
	 * @return array
	 */
	...
	/**
	 * Persist option values using the same sanitization/merge rules as legacy Settings::save.
	...
	private static function apply_settings( array $settings ): array {
		...
		if ( isset( $settings['ip_headers'] ) && is_array( $settings['ip_headers'] ) ) {
			$available = Request::get_ip_headers();
			$options['ip_headers'] = array_filter(
				$settings['ip_headers'],
				fn( $header ) => in_array( $header, $available, true )
			);
			$options['ip_headers'] = array_values( $options['ip_headers'] );
		}

		if ( isset( $settings['rest_api'] ) && in_array( intval( $settings['rest_api'], 10 ), [ 0, 1, 2, 3, 4 ], true ) ) {
		...
		if ( isset( $settings['monitor_types'] ) && count( $monitor_types ) === 0 ) {
			...
			if ( Group::get( $options['monitor_post'] ) === false && $options['monitor_post'] !== 0 ) {
		...
		if ( isset( $settings['last_group_id'] ) ) {
			...
			if ( Group::get( $options['last_group_id'] ) === false ) {
				$groups = Group::get_all();
		...
		if ( isset( $settings['redirect_cache'] ) ) {
			...
			if ( ! in_array( $options['redirect_cache'], [ -1, 0, 1, 24, 24 * 7 ], true ) ) {
		...
		if ( isset( $settings['aliases'] ) && is_array( $settings['aliases'] ) ) {
			$sanitized_aliases = array_map( 'sanitize_text_field', $settings['aliases'] );
			$options['aliases'] = array_values(
				array_filter(
					array_map( 'red_parse_domain_only', $sanitized_aliases ),
		...
		if ( isset( $settings['permalinks'] ) && is_array( $settings['permalinks'] ) ) {
			$options['permalinks'] = array_map(
				fn( $permalink ) => sanitize_option( 'permalink_structure', $permalink ),
				$settings['permalinks']
			);
			$options['permalinks'] = array_values(
				array_filter(
					array_map( 'trim', $options['permalinks'] ),
