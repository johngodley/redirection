 ...
 */
class Plugin extends Api\Route {
	...
	public function permission_callback_manage( \WP_REST_Request $request ) {
		return Redirect_Plugin\Capabilities::has_access( Redirect_Plugin\Capabilities::CAP_SUPPORT_MANAGE );
	}

	public function route_status( \WP_REST_Request $request ) {
		include_once dirname( dirname( __DIR__ ) ) . '/plugin/class-fixer.php';

		$fixer = new Redirect_Plugin\Fixer();
	...
	public function route_fixit( \WP_REST_Request $request ) {
		...
		if ( isset( $params['name'] ) && isset( $params['value'] ) ) {
			...
			if ( $groups === 0 ) {
				Group\Group::create( 'new group', 1 );
			}
		} else {
