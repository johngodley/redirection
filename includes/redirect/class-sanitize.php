...
/**
 * @phpstan-import-type RedirectCreateDetails from Redirect
 */
class Sanitize {
	/**
	...
	private function set_server( $url, array $details ) {
		...
		if ( is_string( $domain ) && $domain !== Request::get_server_name() ) {
	...
	public function get( array $details ) {
		...
		if ( isset( $details['match_data'] ) && isset( $details['match_data']['source'] ) ) {
			...
			$flags = new SourceFlags( $details['match_data']['source'] );
		...
		if ( isset( $details['match_data']['options'] ) && is_array( $details['match_data']['options'] ) ) {
			$source = new SourceOptions( $details['match_data']['options'] );
			$data['match_data']['options'] = $source->get_json();
		}

		$data['match_data'] = array_filter( $data['match_data'] );

		if ( count( $data['match_data'] ) === 0 ) {
			$data['match_data'] = null;
		}

		// Parse URL
		if ( isset( $details['url'] ) && $details['url'] !== '' ) {
			$url = $details['url'];
		} else {
			$url = $this->auto_generate();
		}
		...
		$data['title'] = isset( $details['title'] ) && $details['title'] !== '' ? $details['title'] : null;
		$data['group_id'] = $this->get_group( $details['group_id'] ?? 0 );
		...
		$matcher = Compare::create( isset( $details['match_type'] ) ? sanitize_text_field( $details['match_type'] ) : '' );
		if ( $matcher === null ) {
		...
		$action = Action::create( isset( $details['action_type'] ) ? sanitize_text_field( $details['action_type'] ) : '', $action_code );
		if ( $action === null ) {
		...
		if ( isset( $details['action_data'] ) && is_array( $details['action_data'] ) ) {
			$match_data = $matcher->save( $details['action_data'] ? $details['action_data'] : [], ! $this->is_url_type( $data['action_type'] ) );
	...
	public function is_valid_redirect_code( $code ) {
		return in_array( $code, [ 301, 302, 303, 304, 307, 308 ], true );
	}
	...
	public function is_valid_error_code( $code ) {
		return in_array( $code, [ 400, 401, 403, 404, 410, 418, 451, 500, 501, 502, 503, 504 ], true );
	}
