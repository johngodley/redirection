...
use Redirection\Url\Url;
use Redirection\Url\Matcher;
use Redirection\Url\Query;
use Redirection\Url\SourceFlags;
use Redirection\Compare\Compare;
...
class Redirect {
	 ...
	 * Match object
	 *
	 * @var Compare<array<string, mixed>, (array<string, mixed>|string)>|null
	 */
	public $match = null;
	 ...
	 * @phpstan-param RedirectData $values
	 * @return void
	 */
	private function load_from_data( array $values ) {
		...
		$this->match = Compare::create( $this->match_type, $this->action_data );
		$this->action = Action::create( $this->action_type, $this->action_code );

		$options = Settings::get();
		$this->source_flags = new SourceFlags( array_merge( $options, [ 'flag_regex' => $this->regex ] ) );
		$this->source_options = new SourceOptions();
	...
	public static function get_for_matched_url( $url ) {
		...
		if ( is_array( $rows ) ) {
			...
			usort( $items, [ self::class, 'sort_urls' ] );
	...
	public static function get_old_url( $url ) {
		...
		usort( $items, [ self::class, 'sort_urls_old' ] );
		$items = array_map( [ self::class, 'reduce_sorted_items' ], $items );
	 ...
	 *
	 * @param array{position: int, item: Redirect} $item Item with position and Redirect object.
	 * @return Redirect
	 */
	 ...
	 * Sort URLs
	 *
	 * @param Redirect $first First URL.
	 * @param Redirect $second Second URL.
	 * @return integer
	 */
	...
	public static function disable_where_matches( $url ) {
		...
		$wpdb->update( $wpdb->prefix . 'redirection_items', [ 'status' => 'disabled' ], [ 'url' => $url ] );
	}
	 ...
	 * @phpstan-param RedirectCreateDetails $details
	...
	public static function create( array $details ) {
		...
		if ( isset( $data['match_data'] ) && is_array( $data['match_data'] ) && count( $data['match_data'] ) > 0 ) {
	 ...
	 * @return Action|false true if matched, false otherwise
	 */
	public function get_match( $requested_url, $original_url = false ) {
		...
		// Does the additional Compare logic also match? This provides dynamic checking of things like IP, cookies, etc
		...
		if ( $this->action->needs_target() ) {
			...
			if ( $target_url !== false ) {
				$target_url = Query::add_to_target( $target_url, $original_url, $this->source_flags );
			}
	 ...
	 * @phpstan-param RedirectFilterParams $params
	 * @return boolean
	 */
