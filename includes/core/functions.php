...
function red_is_disabled(): bool {
	return ( defined( 'REDIRECTION_DISABLE' ) && REDIRECTION_DISABLE ) || file_exists( dirname( __DIR__ ) . '/redirection-disable.txt' );
}
...
function red_get_rest_api( $type = false ): string {
	...
	$url = get_rest_url();  // Settings::API_JSON

	if ( $type === Settings::API_JSON_INDEX ) {
		$url = home_url( '/?rest_route=/' );
	} elseif ( $type === Settings::API_JSON_RELATIVE ) {
