...
class Group {
	...
	public static function get_for_select() {
		...
		if ( $rows ) {
			foreach ( $rows as $row ) {
				$module = Module::get( $row->module_id );
	...
	public static function create( $name, $module_id, $enabled = true ) {
		...
		if ( $name !== '' && Module::is_valid_id( $module_id ) ) {
	...
	public function update( $data ) {
		...
		if ( Module::is_valid_id( intval( $data['moduleId'], 10 ) ) ) {
	...
	public function enable() {
		...
		$wpdb->update( $wpdb->prefix . 'redirection_groups', [ 'status' => 'enabled' ], [ 'id' => $this->id ] );
	...
	public function disable() {
		...
		$wpdb->update( $wpdb->prefix . 'redirection_groups', [ 'status' => 'disabled' ], [ 'id' => $this->id ] );
	...
	public static function get_filtered( array $params ) {
		...
		if ( isset( $params['orderby'] ) && in_array( $params['orderby'], [ 'name', 'id' ], true ) ) {
	...
	public function to_dropdown_json() {
		$module = Module::get( $this->get_module_id() );
