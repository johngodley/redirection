...
abstract class Log {
	...
	public function __construct( $values ) {
		...
		if ( isset( $values['agent'] ) ) {
			$this->agent = (string) $values['agent'];
		}

		if ( isset( $values['referrer'] ) ) {
			$this->referrer = (string) $values['referrer'];
		}

		if ( isset( $values['ip'] ) ) {
			$this->ip = (string) $values['ip'];
		}

		if ( isset( $values['domain'] ) ) {
			$this->domain = (string) $values['domain'];
		}
		...
		if ( isset( $values['request_method'] ) ) {
			$this->request_method = (string) $values['request_method'];
		}

		if ( isset( $values['request_data'] ) ) {
			$this->request_data = (string) $values['request_data'];
		}
	 ...
	 *
	 * @param array<string, mixed> $params Array of filter parameters.
	...
	public static function get_query( array $params ) {
		...
		if ( isset( $params['orderby'] ) && in_array( $params['orderby'], [ 'ip', 'url' ], true ) ) {
			$query['orderby'] = $params['orderby'];
		}

		if ( isset( $params['direction'] ) && in_array( strtoupper( $params['direction'] ), [ 'ASC', 'DESC' ], true ) ) {
