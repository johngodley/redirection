 ...
 *
 * @phpstan-extends Compare<IpMap, IpResult>
 */
class Ip extends Compare {
	use FromNotFrom;
	 ...
	 * @phpstan-param IpMap $details
	 * @return IpResult
	 */
	public function save( array $details, $no_target_url = false ) {
		$data = [ 'ip' => isset( $details['ip'] ) && is_array( $details['ip'] ) ? $this->sanitize_ips( $details['ip'] ) : [] ]; // @phpstan-ignore-line
	...
	private function sanitize_ips( array $ips ) {
		$ips = array_map( [ $this, 'sanitize_single_ip' ], $ips );
	...
	public function load( $values ) {
		...
		$this->ip = $data['ip'] ?? []; // @phpstan-ignore-line
	}
}
