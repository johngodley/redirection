 ...
 *     flag_regex: bool
 * }
 * @phpstan-import-type RedirectionOptions from Settings
 */
class SourceFlags {
	...
	public function get_json_with_defaults() {
		...
		// Build a safely-typed set of defaults based on current settings.
		$defaults = [
			self::FLAG_QUERY => self::QUERY_EXACT,
			self::FLAG_CASE => false,
			self::FLAG_TRAILING => false,
			self::FLAG_REGEX => false,
		];

		if ( isset( $settings[ self::FLAG_QUERY ] ) && in_array( $settings[ self::FLAG_QUERY ], $this->get_allowed_query(), true ) ) {
			$defaults[ self::FLAG_QUERY ] = $settings[ self::FLAG_QUERY ];
		}

		if ( isset( $settings[ self::FLAG_CASE ] ) ) {
			$defaults[ self::FLAG_CASE ] = (bool) $settings[ self::FLAG_CASE ];
		}

		if ( isset( $settings[ self::FLAG_TRAILING ] ) ) {
			$defaults[ self::FLAG_TRAILING ] = (bool) $settings[ self::FLAG_TRAILING ];
		}

		if ( isset( $settings[ self::FLAG_REGEX ] ) ) {
			$defaults[ self::FLAG_REGEX ] = (bool) $settings[ self::FLAG_REGEX ];
		}

		if ( ! in_array( self::FLAG_QUERY, $this->values_set, true ) ) {
			$json[ self::FLAG_QUERY ] = $defaults[ self::FLAG_QUERY ];
		}

		if ( ! in_array( self::FLAG_CASE, $this->values_set, true ) ) {
			$json[ self::FLAG_CASE ] = $defaults[ self::FLAG_CASE ];
		}

		if ( ! in_array( self::FLAG_TRAILING, $this->values_set, true ) ) {
			$json[ self::FLAG_TRAILING ] = $defaults[ self::FLAG_TRAILING ];
		}

		if ( ! in_array( self::FLAG_REGEX, $this->values_set, true ) ) {
			$json[ self::FLAG_REGEX ] = $defaults[ self::FLAG_REGEX ];
		}
