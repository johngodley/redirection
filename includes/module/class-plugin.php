...
use Redirection\Url\Request as UrlRequest;
use Redirection\Redirect\Cache;
use Redirection\Redirect\Redirect;
use Redirection\Settings\Settings;
use Redirection\Plugin\Canonical;
use Redirection\Plugin\Permalinks;
use WP_Query;
...
class Plugin extends Module {
	 ...
	 *
	 * @param string $url URL.
	 * @param Redirect|false $matched_redirect Matched redirect.
	 * @param Redirect[] $redirects Array of redirects.
	 * @return void
	 */
	...
	public function pre_handle_404( $result, WP_Query $query ) {
		...
		if ( count( $options['permalinks'] ) > 0 ) {
			include_once dirname( __DIR__ ) . '/wordpress/class-permalinks.php';

			$permalinks = new Permalinks( $options['permalinks'] );
	...
	private function is_url_and_page_type() {
		$page_types = array_values(
			array_filter(
				$this->redirects,
				fn( Redirect $redirect ): bool => $redirect->match !== null && $redirect->match->get_type() === 'page'
			)
		);

		if ( count( $page_types ) > 0 ) {
			$request = new UrlRequest( Request::get_request_url() );
	...
	public function get_canonical_target() {
		$options = Settings::get();
		$canonical = new Canonical( $options['https'], $options['preferred_domain'], $options['aliases'], get_home_url() );

		// Relocate domain?
		if ( strlen( $options['relocate'] ) > 0 ) {
			return $canonical->relocate_request( $options['relocate'], Request::get_server_name(), Request::get_request_url() );
		}

		// Force HTTPS or www
		return $canonical->get_redirect( Request::get_request_server_name(), Request::get_request_url() );
	}
	...
	public function init() {
		...
		if ( $request->is_valid() && ! $request->is_protected_url() ) {
			...
			$redirects = Redirect::get_for_url( $request->get_decoded_url() );
	...
	public function send_headers() {
		if ( $this->matched !== false && $this->matched->action !== null && $this->matched->action->get_code() === 410 ) {
	 ...
	 *
	 * @param Redirect|false $matched Set the `matched` var.
	 * @return void
	 */
