...
export const isRegex = ( text: string ): boolean => {
	if ( text.match( /[\*\\\(\)\^\$]/ ) !== null ) {
		return true;
	}
...
const getRelativeAbsolute = ( url: string ): string | null => {
	const matched = url.match( /^\/([a-zA-Z0-9_\-%]*\..*)\// );

	if ( matched && ! url.includes( 'index.php' ) ) {
		return matched[ 0 ];
	}
...
const beginsWith = ( str: string, match: string ): boolean =>
	match.startsWith( str ) || str.slice( 0, match.length ) === match;

export const getWarningFromState = ( item: any ): React.ReactNode[] => {
	...
	// Server redirect
	if ( url.slice( 0, 4 ) === 'http' && ! url.includes( document.location.origin ) ) {
		try {
				...
				<ExternalLink url="https://redirection.me/support/matching-redirects/#server">
					{ createInterpolateElement(
						sprintf(
							...
							__(
								'This will be converted to a server redirect for the domain {{code}}%(server)s{{/code}}.',
								'redirection'
							),
	...
	if (
		...
		warnings.push(
			createInterpolateElement(
				__( 'The source URL should probably start with a {{code}}/{{/code}}', 'redirection' ),
				{
					code: <code />,
				}
			)
	...
	// Anchor
	if ( ! url.includes( '^' ) && ! url.includes( '$' ) && flag_regex ) {
		warnings.push(
	...
	if ( flag_regex && url.includes( '^' ) && url.indexOf( '^' ) > 0 ) {
		warnings.push(
	...
	if ( flag_regex && url.match( /[a-zA-Z0-9 ]\*/ ) ) {
		warnings.push(
			createInterpolateElement(
				__(
					'Wildcards are not supported. You need to use a {{link}}regular expression{{/link}}.',
					'redirection'
				),
				{
	...
	if ( url === '/(.*)' || url === '^/(.*)' ) {
		warnings.push(
			__(
				'If you want to redirect everything please use a site relocation or alias from the Site page.',
				'redirection'
			)
		);
	}
				...
				warnings.push(
					...
					createInterpolateElement(
						sprintf(
							// translators: %(invalid)s is an invalid character in the URL
							__(
								'Your target URL contains the invalid character {{code}}%(invalid)s{{/code}}',
								'redirection'
							),
							{ invalid: matches }
						),
