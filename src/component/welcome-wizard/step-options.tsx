...
interface StepOptionsProps {
	...
	step: number;
	options: StepOptionsOptions;
	setOptions: ( options: Partial< StepOptionsOptions > ) => void;
}
...
export default function StepOptions( { setStep, step, options, setOptions }: StepOptionsProps ) {
	...
	function setLogChecked( ev: React.ChangeEvent< HTMLInputElement > ) {
		setOptions( {
			settings: {
				...options.settings,
		...
		<>
			...
			<div className="wizard-option">
				...
				<p>
					...
					&nbsp;
					{ createInterpolateElement( __( '{{link}}Read more about this.{{/link}}', 'redirection' ), {
			...
			<div className="wizard-option">
				<p>
					<input id="wizard-log" name="log" type="checkbox" checked={ log } onChange={ setLogChecked } />
					<label htmlFor="wizard-log">
